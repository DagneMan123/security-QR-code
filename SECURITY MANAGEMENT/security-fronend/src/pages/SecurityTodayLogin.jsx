...
const SecurityTodayLogin = () => {
    const { backendUrl, fetchTodayInfo, handleBlockDevice, getDeviceData } = useContext(SecurityContext)
    ...
    const fetchTodayLogins = async () => {
        try {
            ...
            // Filter today logins (same as admin)
            const todayDevices = todayInfo.loginsToday || [];
            
            setDevices(todayDevices);
            
        } catch (error) {
            toast.error("Failed to load today logins");
            console.log(error);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Today's Login Activity</h1>
                    <p className="text-gray-600 dark:text-gray-400 mt-1">Devices that have logged in today</p>
                </div>
            ...
            {loading && (
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-12 shadow border border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">Loading today's login data...</p>
                    </div>
                </div>
            )}
                    ...
                    <div className="text-center">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-100 dark:bg-gray-700 mb-4">
                            <svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12l2 2 4-4m-4 4v-3a4 4 0 118 0v3" />
                            </svg>
                        </div>
                        <h3 className="text-lg font-medium text-gray-900 dark:text-white">No Logins Today</h3>
                        <p className="mt-2 text-gray-500 dark:text-gray-400">No devices have logged in today.</p>
                    </div>
                </div>
            )}
                    ...
                    <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                            <div className="text-sm text-gray-700 dark:text-gray-400 mb-4 sm:mb-0">
                                ...
                                of <span className="font-semibold text-gray-900 dark:text-white">{devices.length}</span> logins
                            </div>
...
export default SecurityTodayLogin;